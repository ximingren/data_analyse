...
class Table():
    ...
    def prepare_env(self, need_split_col, split_threshold, common_merge_threshold):
        ...
        self.create_directory("./result") #创建存放结果集的目录
        self.need_split_col = need_split_col #设置拆分依据列
        self.split_threshold = split_threshold  # 设置专票合并阈值和划分阈值
        self.merge_threshold = {'专票': split_threshold, '普票': common_merge_threshold} #设置普票的专票的合并阈值
    ...
    def pre_process(self):
        ...
        self.fillna_reduced_table()  # 对核减后的表进行填充nan值
    ...
    def add_lack_columns(self):
        ...
        :return:
        """
        self.merged_raw_data = self.merged_raw_data.reindex(columns=self.result_col)  # 对未分组的数据进行处理，设置列排序
        self.merged_raw__duplicate_data = self.merged_raw_data.drop_duplicates(self.group_index) #获取每一组单独的一行
        self.merged_raw__duplicate_data.set_index(self.group_index, inplace=True) #设置层次索引
        self.reduced_data = self.reduced_data.reindex(columns=self.result_col.drop(['购货单位', '产品']))  # 设置列排序（其实是新添多个列）
        self.ripe_data = self.reduced_data.combine_first(self.merged_raw__duplicate_data) #添加缺失的列
        self.ripe_data = self.ripe_data.dropna() #删除含有nan的行
    ...
    def split(self, row_data):
        ...
        if row_data['销售数量'] == 0:
            ...
            self.split_value = row_data['主营业务收入'] / self.split_counts  # 拆分值
            ...
            self.eachData.append(pd.np.nan)  # 差异值
        self.diff_result_col = self.result_col.append(pd.Index(['差异'])) #构造具有差异一列的索引
        self.eachData = pd.Series(self.eachData, index=self.diff_result_col)  # 拆分值的每一行
    ...
    def integrate(self, row_data):
        ...
        :return:
        """
        self.total_afterTax = 0 #拆分后的总销项税额
        for k in range(self.split_counts):
            ...
            if k != self.split_counts - 1:
                self.total_afterTax += self.eachData[self.param_len + 2]
            else:
                # 要销售数量全部加起来等于原数据的值,总销售数量-总的划分销售数量=最后一行的销售数量
                self.eachData[self.param_len] = row_data['销售数量'] - self.eachData[self.param_len] * (
                        self.split_counts - 1)
                ...
                self.eachData[self.param_len + 4] = float(
                    '%.2f' % (row_data['销项税额'] - self.total_afterTax))
            self.each_split_result = self.each_split_result.append(self.eachData, ignore_index=True) #将每一行添加到结果集中
...
def process_data_main(sheet_name, lock):
    ...
    lock.acquire()  # 获取锁
    split_data_list.append(table.ripe_data)  # 将结果集加入到共享变量中
...
def export_data(no_split_file, splited_file):
    ...
    """
    try:
        no_split_writer = pd.ExcelWriter(no_split_file)  # 拆分前导出的文件
        split_writer = pd.ExcelWriter(splited_file)  # 最后的结果集导出的文件
...
if __name__ == '__main__':
    try:
        ...
        export_before_split_data_file = r".\result\before_split_total_data1.xls"
        export_result_file = r".\result\final_result1.xls"
        manager = Manager() #为了让多进程共享变量
        split_data_list = manager.list()  # 存放未拆分前的总数据
        final_data_list = manager.list()  # 存放最后的结果集
        sheet_names_list = manager.list() # 存放sheet的名称
        ...
        export_data(export_before_split_data_file, export_result_file)  # 导出数据表
    except Exception as e:
        print(e)
